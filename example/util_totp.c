...
int main(int argc, char *argv[])
{
	...
	while ((r = conch_getopt_r(argc, argv, "-hs:l:", &arg, &ind)) != -1) {
		switch (r) {
			case 1:
				...
				len = (uint32_t)conch_strlen(arg);
				if (len > BASE32_DEC_IALIGN(128)) {
					printf("secret length too long!\n");
					return 1;
				}

				r = conch_base32_dec(arg, len, buf, &buf_len);
				if (r) {
				...
				break;
			case 's':
				ts = (uint32_t)conch_atoi(arg);
				arg = NULL;
				break;
			case 'l':
				cl = (uint32_t)conch_atoi(arg);
				if (cl > 8) {
