		...
		uint8_t *okm, uint32_t len, int32_t type)
{
	...
	conch_hmac_sha3_init(&ctx, salt, salt_len, type);
	conch_hmac_sha3_process(&ctx, ikm, ikm_len);
	conch_hmac_sha3_finish(&ctx);
	conch_memcpy(prk, &(HMAC_SHA3_STATE(&ctx, 0)), dsize);

	uint32_t n = (len + dsize - 1) / dsize;
	for (uint32_t i = 1; i <= n; i++) {
		count[0] = i & 0xff;

		conch_hmac_sha3_init(&ctx, prk, dsize, type);
		conch_hmac_sha3_process(&ctx, tmp, tmp_len);
		conch_hmac_sha3_process(&ctx, info, info_len);
		conch_hmac_sha3_process(&ctx, count, 1);
		conch_hmac_sha3_finish(&ctx);
		tmp_len = dsize;

		conch_memcpy(tmp, &(HMAC_SHA3_STATE(&ctx, 0)), dsize);
		conch_memcpy(okm + f, tmp, ((f + dsize) > len) ?
			(len % dsize) : dsize);
		f += dsize;
	}
