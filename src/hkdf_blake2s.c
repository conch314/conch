		...
		uint8_t *okm, uint32_t len)
{
	...
	conch_hmac_blake2s_init(&ctx, salt, salt_len);
	conch_hmac_blake2s_process(&ctx, ikm, ikm_len);
	conch_hmac_blake2s_finish(&ctx);
	conch_memcpy(prk, &(HMAC_BLAKE2S_STATE(&ctx, 0)),
		BLAKE2S_256_LEN);
	...
	for (uint32_t i = 1; i <= n; i++) {
		...
		conch_hmac_blake2s_init(&ctx, prk, BLAKE2S_256_LEN);
		conch_hmac_blake2s_process(&ctx, tmp, tmp_len);
		conch_hmac_blake2s_process(&ctx, info, info_len);
		conch_hmac_blake2s_process(&ctx, count, 1);
		conch_hmac_blake2s_finish(&ctx);
		tmp_len = BLAKE2S_256_LEN;

		conch_memcpy(tmp, &(HMAC_BLAKE2S_STATE(&ctx, 0)),
			BLAKE2S_256_LEN);
		conch_memcpy(okm + f, tmp, ((f + BLAKE2S_256_LEN) > len) ?
