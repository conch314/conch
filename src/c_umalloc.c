 ...
 *
 * #1: ctx  [in/out] umalloc context struct
 * #2: size [in]     chunk size
 * #r:      [ret]    chunk pointer / NULL
 */
static void *_umalloc_new_node(struct umalloc_ctx *ctx, uint32_t size)
{
	...
	conch_list_add(&ctx->chunk, &node->list);
	ctx->idle = &node->chunk;
 ...
 *
 * #1: ctx   [in/out] umalloc context struct
 * #2: chunk [in/out] chunk pointer
 * #3: size  [in]     allocate size
 * #r:       [ret]    allocated chunk pointer / NULL
 */
 ...
 *
 * #1: ctx  [in/out] umalloc context struct
 * #2: size [in]     allocate size
 * #r:      [ret]    allocated pointer / NULL
 */
...
/* @func: conch_umalloc_free
 * #desc:
 *    free allocated memory.
 *
 * #1: ctx [in/out] umalloc context struct
 * #2: p   [in/out] allocated pointer
 * #r:     [ret]    0: no error, -1: double free, -2: callback error
 */
