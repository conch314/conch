...
void conch_avl_erase_fix(struct avl_root *root, struct avl_node *node)
{
	...
	if (left && right) {
		...
		if (successor == right) {
			conch_avl_balance(root, left);
		} else {
			conch_avl_balance(root, successor);
		}
	...
	if (parent) {
		/* sibling node */
		if (parent->left) {
			conch_avl_balance(root, parent->left);
		} else if (parent->right) {
			conch_avl_balance(root, parent->right);
		} else { /* parent */
			conch_avl_balance(root, parent);
		}
	} /* 'parent' is nil == root */
}

/* @func: conch_avl_insert
 * #desc:
		...
		struct avl_node *new, int32_t (*cmp)(void *, void *))
{
	...
	new->bf = 1;
	*link = new;
	conch_avl_balance(root, new);

	return new;
}

/* @func: conch_avl_search
 * #desc:
...
{
	if (old)
		conch_avl_erase_fix(root, old);

	return old;
}

/* @func: conch_avl_erase2
 * #desc:
...
struct avl_node *conch_avl_erase2(struct avl_root *root,
		void *arg, int32_t (*cmp)(void *, void *))
{
	struct avl_node *old = conch_avl_search(root, arg, cmp);
	if (old)
		conch_avl_erase_fix(root, old);
