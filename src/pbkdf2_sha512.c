		...
		uint32_t len, uint32_t k)
{
	...
	for (uint32_t i = 1; i <= n; i++) {
		...
		conch_hmac_sha512_init(&ctx, pass, pass_len);
		conch_hmac_sha512_process(&ctx, salt, salt_len);
		conch_hmac_sha512_process(&ctx, count, 4);
		conch_hmac_sha512_finish(&ctx, salt_len + 4);
		conch_memcpy(u, &(HMAC_SHA512_STATE(&ctx, 0)),
			SHA512_LEN);
		...
		for (uint32_t j = 1; j < k; j++) {
			conch_hmac_sha512_init(&ctx, pass, pass_len);
			conch_hmac_sha512_process(&ctx, u, SHA512_LEN);
			conch_hmac_sha512_finish(&ctx, SHA512_LEN);
			conch_memcpy(u, &(HMAC_SHA512_STATE(&ctx, 0)),
				SHA512_LEN);
		...
		conch_memcpy(odk + f, tmp, ((f + SHA512_LEN) > len) ?
			(len % SHA512_LEN) : SHA512_LEN);
		f += SHA512_LEN;
	}
}

/* @func: conch_pbkdf2_sha384
 * #desc:
		...
		uint32_t len, uint32_t k)
{
	...
	for (uint32_t i = 1; i <= n; i++) {
		...
		conch_hmac_sha384_init(&ctx, pass, pass_len);
		conch_hmac_sha384_process(&ctx, salt, salt_len);
		conch_hmac_sha384_process(&ctx, count, 4);
		conch_hmac_sha384_finish(&ctx, salt_len + 4);
		conch_memcpy(u, &(HMAC_SHA384_STATE(&ctx, 0)),
			SHA384_LEN);
		...
		for (uint32_t j = 1; j < k; j++) {
			conch_hmac_sha384_init(&ctx, pass, pass_len);
			conch_hmac_sha384_process(&ctx, u, SHA384_LEN);
			conch_hmac_sha384_finish(&ctx, SHA384_LEN);
			conch_memcpy(u, &(HMAC_SHA384_STATE(&ctx, 0)),
				SHA384_LEN);
		...
		conch_memcpy(odk + f, tmp, ((f + SHA384_LEN) > len) ?
