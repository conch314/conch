...
{
	MD5_NEW(md5_ctx);
	conch_md5_init(&md5_ctx);

	if (key_len > MD5_BLOCKSIZE) {
		conch_md5(&md5_ctx, key, key_len);
		key = &(MD5_STATE(&md5_ctx, 0));
		key_len = MD5_LEN;
	}

	conch_memset(ctx->ipad, 0x36, MD5_BLOCKSIZE);
	conch_memset(ctx->opad, 0x5c, MD5_BLOCKSIZE);
	for (uint32_t i = 0; i < key_len; i++) {
		ctx->ipad[i] ^= key[i];
		ctx->opad[i] ^= key[i];
	}

	conch_md5_init(&ctx->ipad_ctx);
	conch_md5_init(&ctx->opad_ctx);
	conch_md5_process(&ctx->ipad_ctx,
		ctx->ipad, MD5_BLOCKSIZE);

	ctx->state = &(MD5_STATE(&ctx->opad_ctx, 0));
}

/* @func: conch_hmac_md5_process
 * #desc:
...
void conch_hmac_md5_process(struct hmac_md5_ctx *ctx, const uint8_t *s,
		size_t len)
{
	conch_md5_process(&ctx->ipad_ctx, s, len);
}

/* @func: conch_hmac_md5_finish
 * #desc:
...
{
	conch_md5_finish(&ctx->ipad_ctx, len + MD5_BLOCKSIZE);

	conch_md5_process(&ctx->opad_ctx,
		ctx->opad, MD5_BLOCKSIZE);
	conch_md5_process(&ctx->opad_ctx,
		&(MD5_STATE(&ctx->ipad_ctx, 0)), MD5_LEN);
	conch_md5_finish(&ctx->opad_ctx, MD5_BLOCKSIZE + MD5_LEN);
}
...
void conch_hmac_md5(struct hmac_md5_ctx *ctx, const uint8_t *s,
		size_t len)
{
	conch_hmac_md5_process(ctx, s, len);
	conch_hmac_md5_finish(ctx, len);
}
