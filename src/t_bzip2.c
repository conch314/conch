...
#define BZIP2_BLOCKSIZE_MAX (900000 + 16)
...
#define BZIP2_BUFSIZE (BZIP2_BLOCKSIZE_MAX * 2 + 150000)
...
struct bzip2_ctx {
	uint8_t block[BZIP2_BLOCKSIZE_MAX];
	...
	uint32_t sort_sa[BZIP2_BLOCKSIZE_MAX];
	...
	uint32_t mtf_n;
	uint32_t mtf_e;
	uint32_t mtf_freq[258];

	uint8_t huf_len[6][258];
	uint32_t huf_code[6][258];
	uint32_t huf_rfreq[6][258];
 ...
 * #1: ctx [in/out] bzip2 struct context
 */
static void _send_bits_skip(struct bzip2_ctx *ctx)
{
	BITS_ADD_SKIP(&ctx->bits_ctx);
}

static void _build_codelen(uint8_t *lens, uint32_t *freq, int32_t alpha_size,
		int32_t max_len)
{
	int32_t heap[258 + 2];
	int32_t weight[258 * 2];
...
static void _gen_codes(uint32_t *code, uint8_t *lens,
		int32_t min_len, int32_t max_len, int32_t alpha_size)
{
	int32_t v = 0;
 ...
 * #desc:
 *    cyclic suffix array for fallback sort.  doubling method, bucket
 *    partitioning, and 3-way quicksort.
 *
 ...
 * #desc:
 *    cyclic suffix array for block sort.
 *
 * #1: ctx [in/out] bzip2 struct context
 */
 ...
 * #desc:
 *    generate mtf (move-to-front) value.
 *
 * #1: ctx [in/out] bzip2 struct context
 */
...
static void _send_block(struct bzip2_ctx *ctx)
{
	...
	ctx->groups_n = ngroups;
	ctx->selector_n = nselectors;
 ...
 */
static int32_t _compress(struct bzip2_ctx *ctx, const uint8_t *s,
		...
		uint32_t len, int32_t flush)
{
	...
	return _compress(ctx, s, len, flush);
}
/*
static struct bzip2_ctx ctx;

int main(void)
{
	conch_bzip2_init(&ctx, 6);
	char *s = "bananaaaaaaa\003bbbbbbbbaaaa\n";

	conch_bzip2(&ctx, (uint8_t *)s, conch_strlen(s), 0);
	conch_bzip2(&ctx, NULL, 0, 1);

// "bananaaaa\003bbbb\004aaaa\000\n"
...
static BZIP2_NEW(ctx);

static int32_t __compress(FILE *rfp, FILE *wfp, int32_t lev, int32_t is_v)
{
	uint8_t buf[8192];
	size_t total_len = 0, send_len = 0, len;
	int32_t r;

	if (conch_bzip2_init(&ctx, lev)) {
		fprintf(stderr, "bzip2_init() initialize error!\n");
		return -1;
	}
	...
	while ((len = fread(buf, 1, sizeof(buf), rfp))) {
		do {
			r = conch_bzip2(&ctx, buf, len, 0);
			if (r) {
				send_len += BZIP2_LEN(&ctx);
				fwrite(BZIP2_BUF(&ctx),
					1, BZIP2_LEN(&ctx), wfp);
			}
		} while (r);
...
int main(int argc, char *argv[])
{
	...
	while ((r = conch_getopt_r(argc, argv, "hv123456789",
			&arg, &ind)) != -1) {
		switch (r) {
	...
	if (__compress(stdin, stdout, lev, is_v))
		return 1;
