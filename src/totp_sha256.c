/* @file: totp_sha256.c
 * #desc:
 *    The implementations of time-based one-time password (sha256).
 *
 * #copy:
...
/* @func: conch_totp_sha256
 * #desc:
 *    totp-sha256 one-time password function.
 *
 * #1: key       [in]  input key
 ...
 */
uint32_t conch_totp_sha256(const uint8_t *key, uint32_t key_len,
		uint64_t time_base, uint32_t n)
{
	HMAC_SHA256_NEW(ctx);
	uint8_t tmp[SHA256_LEN], base[8];
	...
	conch_hmac_sha256_init(&ctx, key, key_len);
	conch_hmac_sha256(&ctx, base, sizeof(base));
	conch_memcpy(tmp, &(HMAC_SHA256_STATE(&ctx, 0)), SHA256_LEN);

	uint32_t f = tmp[SHA256_LEN - 1] & 0x0f;
