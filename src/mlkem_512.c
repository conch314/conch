...
#define MLKEM_MONT_R2 1353 /* 2^32 == 1353 (mod q) */
#define MLKEM_QINV -3327   /* q^-1 == 62209 (mod 2^16) */
#define MLKEM_NORM 1441    /* mont^2*128^-1 == 1441 (mod q) */
...
{
	for (int32_t i = 0; i < MLKEM_512_K; i++) {
		_poly_compress_du(&a->vec[i], r);
		r += MLKEM_512_POLY_COMPRESS_DU;
	}
}
		...
		const struct polyvec *a, const struct polyvec *b)
{
	...
	for (int32_t i = 1; i < MLKEM_512_K; i++) {
		...
		uint8_t x, uint8_t y)
{
	...
	conch_sha3_process(&ctx, seed, 32);
	conch_sha3_process(&ctx, buf, 2);
	conch_sha3_finish(&ctx);

	uint32_t n = 0;
	do {
		conch_sha3_shake_xof(&ctx, buf, sizeof(buf));
...
/* @func: _poly_cbd3 (static)
 * #desc:
 *    extract the polynomial of the central binomial distribution 3.
 *
 * #1: r   [out] output polynomial
 * #2: buf [in]  input buffer
 */
static void _poly_cbd3(struct poly *r, const uint8_t *buf)
{
	uint32_t t, d;
	int16_t a, b;
...
static void _sample_poly_eta1_prf(struct poly *r, const uint8_t *seed,
		uint8_t ran)
{
	uint8_t buf[MLKEM_512_ETA1 * MLKEM_N / 4];
	SHA3_NEW(ctx);

	buf[0] = ran;
	conch_sha3_init(&ctx, SHA3_SHAKE256_TYPE, 0);
	conch_sha3_process(&ctx, seed, MLKEM_SYM_LEN);
	conch_sha3_process(&ctx, buf, 1);
	conch_sha3_finish(&ctx);
	conch_sha3_shake_xof(&ctx, buf, sizeof(buf));
...
static void _sample_poly_eta2_prf(struct poly *r, const uint8_t *seed,
		uint8_t ran)
{
	uint8_t buf[MLKEM_512_ETA2 * MLKEM_N / 4];
	SHA3_NEW(ctx);
...
static void _hash_h(const uint8_t *in, uint32_t len, uint8_t *out)
{
	...
	conch_sha3_process(&ctx, in, len);
	conch_sha3_finish(&ctx);
	conch_memcpy(out, &(SHA3_STATE(&ctx, 0)), SHA3_256_LEN);
}
...
static void _hash_g(const uint8_t *in, uint32_t len, uint8_t *out)
{
	...
	conch_sha3_process(&ctx, in, len);
	conch_sha3_finish(&ctx);
	conch_memcpy(out, &(SHA3_STATE(&ctx, 0)), SHA3_512_LEN);
}
...
static void _hash_j(const uint8_t *in, uint32_t len, uint8_t *out)
{
	...
	conch_sha3_init(&ctx, SHA3_SHAKE256_TYPE, MLKEM_SYM_LEN);
	conch_sha3_process(&ctx, in, len);
	conch_sha3_finish(&ctx);
	conch_memcpy(out, &(SHA3_STATE(&ctx, 0)), MLKEM_SYM_LEN);
}

/* @func: conch_mlkem512_pke_genkey
 * #desc:
		...
		uint8_t *dkp)
{
	...
	conch_memcpy(ekp + MLKEM_512_POLYVEC_BYTES, seed, MLKEM_SYM_LEN);
	/* dkp = s */
	_polyvec_tobytes(&s, dkp);
}

/* @func: conch_mlkem512_pke_encrypt
 * #desc:
		...
		const uint8_t *msg, uint8_t *ct)
{
	...
	struct polyvec A[MLKEM_512_K], r, e, u, t;
	struct poly e2, v, m;

	/* (t, seed) = ekp */
	_polyvec_frombytes(&t, ekp);
	conch_memcpy(seed, ekp + MLKEM_512_POLYVEC_BYTES, MLKEM_SYM_LEN);
	/* m = msg */
	_poly_frommsg(&m, msg);
	...
	_poly_compress_dv(&v, ct + MLKEM_512_POLYVEC_COMPRESS);
}

/* @func: conch_mlkem512_pke_decrypt
 * #desc:
		...
		uint8_t *msg)
{
	...
	_poly_decompress_dv(ct + MLKEM_512_POLYVEC_COMPRESS, &v);
		...
		uint8_t *ek, uint8_t *dk)
{
	...
	conch_mlkem512_pke_genkey(ran, ek, dk);

	/* dk = dkp + ekp + hash_h(ekp) + ran2 */
	dk += MLKEM_512_DKPKE_LEN;
	conch_memcpy(dk, ek, MLKEM_512_EK_LEN);

	dk += MLKEM_512_EK_LEN;
	_hash_h(ek, MLKEM_512_EK_LEN, dk);

	dk += MLKEM_SYM_LEN;
	conch_memcpy(dk, ran2, MLKEM_RAN_LEN);
}

/* @func: conch_mlkem512_encaps
 * #desc:
		...
		uint8_t *ct, uint8_t *sk)
{
	...
	_hash_h(ek, MLKEM_512_EK_LEN, buf + MLKEM_SYM_LEN);
	_hash_g(buf, MLKEM_SYM_LEN * 2, buf);

	/* sk = k */
	conch_memcpy(sk, k, MLKEM_KEY_LEN);

	/* ct = pke_encrypt(r, ek, msg) */
	conch_mlkem512_pke_encrypt(r, ek, msg, ct);
}

/* @func: conch_mlkem512_decaps
 * #desc:
...
void conch_mlkem512_decaps(const uint8_t *dk, const uint8_t *ct,
		uint8_t *sk)
{
	uint8_t buf[MLKEM_SYM_LEN + MLKEM_512_CT_LEN],
		ct2[MLKEM_512_CT_LEN], sk2[MLKEM_SYM_LEN],
		msg[MLKEM_SYM_LEN];
	const uint8_t *k = buf;
	const uint8_t *r = buf + MLKEM_SYM_LEN;
	const uint8_t *dkp = dk;
	const uint8_t *ekp = dk += MLKEM_512_DKPKE_LEN;
	const uint8_t *h = dk += MLKEM_512_EK_LEN;
	const uint8_t *z = dk += MLKEM_SYM_LEN;

	/* sk2 = hash_j(z + ct) */
	conch_memcpy(buf, z, MLKEM_RAN_LEN);
	conch_memcpy(buf, ct, MLKEM_512_CT_LEN);
	_hash_j(buf, MLKEM_RAN_LEN + MLKEM_512_CT_LEN, sk2);

	/* msg = pke_decrypt(dkp, ct) */
	conch_mlkem512_pke_decrypt(dkp, ct, msg);

	/* (k, r) = hash_g(msg + h) */
	conch_memcpy(buf, msg, MLKEM_SYM_LEN);
	conch_memcpy(buf + MLKEM_SYM_LEN, h, MLKEM_SYM_LEN);
	_hash_g(buf, MLKEM_SYM_LEN * 2, buf);
	...
	conch_mlkem512_pke_encrypt(r, ekp, msg, ct2);

	/* sk = if ct != ct2 : swap sk2 */
	uint32_t rr = 0;
	for (int32_t i = 0; i < MLKEM_512_CT_LEN; i++)
		rr |= ct[i] ^ ct2[i];

	rr = (((rr - 1) >> 31) & 1) - 1;
	for (int32_t i = 0; i < MLKEM_KEY_LEN; i++) {
		uint32_t t = rr & (sk[i] ^ sk2[i]);
		sk[i] ^= t;
		sk2[i] ^= t;
	}
}
