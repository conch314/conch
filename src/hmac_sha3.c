		...
		const uint8_t *key, uint32_t key_len, int32_t type)
{
	...
	if (conch_sha3_init(&sha3_ctx, type, 0))
		return -1;

	if (key_len > SHA3_GETRATE(&sha3_ctx)) {
		conch_sha3(&sha3_ctx, key, key_len);
		key = &(SHA3_STATE(&sha3_ctx, 0));
		key_len = SHA3_GETDSIZE(&sha3_ctx);
	}

	conch_memset(ctx->ipad, 0x36, SHA3_STATE_SIZE);
	conch_memset(ctx->opad, 0x5c, SHA3_STATE_SIZE);
	for (uint32_t i = 0; i < key_len; i++) {
		ctx->ipad[i] ^= key[i];
		ctx->opad[i] ^= key[i];
	}

	if (conch_sha3_init(&ctx->ipad_ctx, type, 0))
		return -1;
	if (conch_sha3_init(&ctx->opad_ctx, type, 0))
		return -1;

	conch_sha3_process(&ctx->ipad_ctx,
		ctx->ipad, SHA3_GETRATE(&ctx->ipad_ctx));
...
void conch_hmac_sha3_process(struct hmac_sha3_ctx *ctx, const uint8_t *s,
		size_t len)
{
	conch_sha3_process(&ctx->ipad_ctx, s, len);
}

/* @func: conch_hmac_sha3_finish
 * #desc:
...
{
	conch_sha3_finish(&ctx->ipad_ctx);

	conch_sha3_process(&ctx->opad_ctx,
		ctx->opad, SHA3_GETRATE(&ctx->opad_ctx));
	conch_sha3_process(&ctx->opad_ctx,
		&(SHA3_STATE(&ctx->ipad_ctx, 0)),
		SHA3_GETDSIZE(&ctx->ipad_ctx));
	conch_sha3_finish(&ctx->opad_ctx);
}

/* @func: conch_hmac_sha3
 * #desc:
...
void conch_hmac_sha3(struct hmac_sha3_ctx *ctx, const uint8_t *s,
		size_t len)
{
	conch_hmac_sha3_process(ctx, s, len);
