...
/* @func: conch_keccak_f1600
 * #desc:
 *    keccak-f permutation function.
 *
 * #1: state [in/out] state buffer
 */
void conch_keccak_f1600(uint64_t state[5][5])
{
		...
		size_t len, uint32_t rate)
{
	while (len >= rate) {
		...
		conch_keccak_f1600(state);
	}

	return len;
}

/* @func: conch_keccak_squeeze
 * #desc:
 ...
 */
void conch_keccak_squeeze(uint64_t state[5][5], uint8_t *out,
 ...
 */
int32_t conch_sha3_init(struct sha3_ctx *ctx, int32_t type,
		uint32_t dsize)
{
	ctx->pad = 0x06;
	switch (type) {
	...
	conch_memset(ctx->state, 0, sizeof(ctx->state));
	ctx->count = 0;

	return 0;
}

/* @func: conch_sha3_process
 * #desc:
 ...
 * #3: len [in]     input length
 */
void conch_sha3_process(struct sha3_ctx *ctx, const uint8_t *s,
		size_t len)
{
	...
	switch (n) {
		default:
			h = ctx->rate - n;
			h = (len < h) ? len : h;
			conch_memcpy(ctx->u.buf + n, s, h);
			n += h;
			if (n != ctx->rate)
				break;

			/* processing */
			conch_keccak_absorb(ctx->state, ctx->u.buf,
				ctx->rate, ctx->rate);
			n = 0;
			s += h;
			len -= h;
		case 0:
			/* processing */
			n = conch_keccak_absorb(ctx->state, s, len,
				ctx->rate);
			if (n) {
				s += len - n;
				conch_memcpy(ctx->u.buf, s, n);
			}
	}
	ctx->count = n;
}

/* @func: conch_sha3_finish
 * #desc:
...
void conch_sha3_finish(struct sha3_ctx *ctx)
{
	...
	conch_keccak_absorb(ctx->state, ctx->u.buf, ctx->rate,
		ctx->rate);
	conch_keccak_squeeze(ctx->state, ctx->u.digest, ctx->dsize,
		ctx->rate);
}
 ...
 * #3: len [in]     input length
 */
void conch_sha3(struct sha3_ctx *ctx, const uint8_t *s, size_t len)
{
	conch_sha3_process(ctx, s, len);
	conch_sha3_finish(ctx);
}

/* @func: conch_sha3_shake_xof
 * #desc:
 ...
 * #3: len [in]     digest length
 */
void conch_sha3_shake_xof(struct sha3_ctx *ctx, uint8_t *out, uint32_t len)
{
	conch_keccak_squeeze(ctx->state, out, len, ctx->rate);
}
