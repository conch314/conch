...
/* aes TD0 0x0e.iS^0x09.iS^0x0d.iS^0x0b.iS */
...
/* aes TD1 0x0b.iS^0x0e.iS^0x09.iS^0x0d.iS */
...
/* aes TD2 0x0d.iS^0x0b.iS^0x0e.iS^0x09.iS */
...
/* aes TD3 0x09.iS^0x0d.iS^0x0b.iS^0x0e.iS */
...
#define AES_ROTSUB(x) \
	((aes_TE4[((x) >> 16) & 0xff] & 0xff000000) \
	^ (aes_TE4[((x) >> 8) & 0xff] & 0x00ff0000) \
	^ (aes_TE4[(x) & 0xff] & 0x0000ff00) \
	^ (aes_TE4[((x) >> 24) & 0xff] & 0x000000ff))

#define AES_DEC_KEYEXP(x) \
	(aes_TD0[aes_TE1[(x) >> 24] & 0xff] \
	^ aes_TD1[aes_TE1[((x) >> 16) & 0xff] & 0xff] \
	^ aes_TD2[aes_TE1[((x) >> 8) & 0xff] & 0xff] \
	^ aes_TD3[aes_TE1[(x) & 0xff] & 0xff])

#define AES_ROUND_INIT(out, state, keyexp) \
	out[0] = PACK4_BE(state) ^ keyexp[0]; \
	out[1] = PACK4_BE(state + 4) ^ keyexp[1]; \
	out[2] = PACK4_BE(state + 8) ^ keyexp[2]; \
	out[3] = PACK4_BE(state + 12) ^ keyexp[3]

#define AES_OUT(state, in) \
	UNPACK4_BE(state, in[0]); \
	UNPACK4_BE(state + 4, in[1]); \
	UNPACK4_BE(state + 8, in[2]); \
	UNPACK4_BE(state + 12, in[3])

#define AES_ENC_ROUND(out, in, keyexp) \
	out[0] = aes_TE0[in[0] >> 24] \
		^ aes_TE1[(in[1] >> 16) & 0xff] \
		^ aes_TE2[(in[2] >> 8) & 0xff] \
		^ aes_TE3[in[3] & 0xff] \
		^ keyexp[0]; \
	out[1] = aes_TE0[in[1] >> 24] \
		^ aes_TE1[(in[2] >> 16) & 0xff] \
		^ aes_TE2[(in[3] >> 8) & 0xff] \
		^ aes_TE3[in[0] & 0xff] \
		^ keyexp[1]; \
	out[2] = aes_TE0[in[2] >> 24] \
		^ aes_TE1[(in[3] >> 16) & 0xff] \
		^ aes_TE2[(in[0] >> 8) & 0xff] \
		^ aes_TE3[in[1] & 0xff] \
		^ keyexp[2]; \
	out[3] = aes_TE0[in[3] >> 24] \
		^ aes_TE1[(in[0] >> 16) & 0xff] \
		^ aes_TE2[(in[1] >> 8) & 0xff] \
		^ aes_TE3[in[2] & 0xff] \
		^ keyexp[3]

#define AES_ENC_END(out, in, keyexp) \
	out[0] = (aes_TE4[in[0] >> 24] & 0xff000000) \
		^ (aes_TE4[(in[1] >> 16) & 0xff] & 0x00ff0000) \
		^ (aes_TE4[(in[2] >> 8) & 0xff] & 0x0000ff00) \
		^ (aes_TE4[in[3] & 0xff] & 0x000000ff) \
		^ keyexp[0]; \
	out[1] = (aes_TE4[in[1] >> 24] & 0xff000000) \
		^ (aes_TE4[(in[2] >> 16) & 0xff] & 0x00ff0000) \
		^ (aes_TE4[(in[3] >> 8) & 0xff] & 0x0000ff00) \
		^ (aes_TE4[in[0] & 0xff] & 0x000000ff) \
		^ keyexp[1]; \
	out[2] = (aes_TE4[in[2] >> 24] & 0xff000000) \
		^ (aes_TE4[(in[3] >> 16) & 0xff] & 0x00ff0000) \
		^ (aes_TE4[(in[0] >> 8) & 0xff] & 0x0000ff00) \
		^ (aes_TE4[in[1] & 0xff] & 0x000000ff) \
		^ keyexp[2]; \
	out[3] = (aes_TE4[in[3] >> 24] & 0xff000000) \
		^ (aes_TE4[(in[0] >> 16) & 0xff] & 0x00ff0000) \
		^ (aes_TE4[(in[1] >> 8) & 0xff] & 0x0000ff00) \
		^ (aes_TE4[in[2] & 0xff] & 0x000000ff) \
		^ keyexp[3]

#define AES_DEC_ROUND(out, in, keyexp) \
	out[0] = aes_TD0[in[0] >> 24] \
		^ aes_TD1[(in[3] >> 16) & 0xff] \
		^ aes_TD2[(in[2] >> 8) & 0xff] \
		^ aes_TD3[in[1] & 0xff] \
		^ keyexp[0]; \
	out[1] = aes_TD0[in[1] >> 24] \
		^ aes_TD1[(in[0] >> 16) & 0xff] \
		^ aes_TD2[(in[3] >> 8) & 0xff] \
		^ aes_TD3[in[2] & 0xff] \
		^ keyexp[1]; \
	out[2] = aes_TD0[in[2] >> 24] \
		^ aes_TD1[(in[1] >> 16) & 0xff] \
		^ aes_TD2[(in[0] >> 8) & 0xff] \
		^ aes_TD3[in[3] & 0xff] \
		^ keyexp[2]; \
	out[3] = aes_TD0[in[3] >> 24] \
		^ aes_TD1[(in[2] >> 16) & 0xff] \
		^ aes_TD2[(in[1] >> 8) & 0xff] \
		^ aes_TD3[in[0] & 0xff] \
		^ keyexp[3]

#define AES_DEC_END(out, in, keyexp) \
	out[0] = (aes_TD4[in[0] >> 24] & 0xff000000) \
		^ (aes_TD4[(in[3] >> 16) & 0xff] & 0x00ff0000) \
		^ (aes_TD4[(in[2] >> 8) & 0xff] & 0x0000ff00) \
		^ (aes_TD4[in[1] & 0xff] & 0x000000ff) \
		^ keyexp[0]; \
	out[1] = (aes_TD4[in[1] >> 24] & 0xff000000) \
		^ (aes_TD4[(in[0] >> 16) & 0xff] & 0x00ff0000) \
		^ (aes_TD4[(in[3] >> 8) & 0xff] & 0x0000ff00) \
		^ (aes_TD4[in[2] & 0xff] & 0x000000ff) \
		^ keyexp[1]; \
	out[2] = (aes_TD4[in[2] >> 24] & 0xff000000) \
		^ (aes_TD4[(in[1] >> 16) & 0xff] & 0x00ff0000) \
		^ (aes_TD4[(in[0] >> 8) & 0xff] & 0x0000ff00) \
		^ (aes_TD4[in[3] & 0xff] & 0x000000ff) \
		^ keyexp[2]; \
	out[3] = (aes_TD4[in[3] >> 24] & 0xff000000) \
		^ (aes_TD4[(in[2] >> 16) & 0xff] & 0x00ff0000) \
		^ (aes_TD4[(in[1] >> 8) & 0xff] & 0x0000ff00) \
		^ (aes_TD4[in[0] & 0xff] & 0x000000ff) \
		^ keyexp[3]


/* @func: _aes_keyexp (static)
 * #desc:
...
{
	uint32_t *keyexp = (uint32_t *)ctx->keyexp;
	uint32_t *dec_keyexp = (uint32_t *)ctx->dec_keyexp;

	for (int32_t i = 0; i < ctx->keylen; i += 4)
		keyexp[i / 4] = PACK4_BE(key + i);

	for (int32_t i = ctx->keylen;
			i < (AES_BLOCKSIZE * (ctx->rounds + 1));
			i += 4) {
		uint32_t tmp = keyexp[i / 4 - 1];

		if (!(i % ctx->keylen)) {
			tmp = AES_ROTSUB(tmp);
			tmp ^= aes_rcon[i / ctx->keylen];
		}

		int32_t k = i - ctx->keylen;
		keyexp[i / 4] = keyexp[k / 4] ^ tmp;
	}

	conch_memcpy(ctx->dec_keyexp, ctx->keyexp, AES_KEYEXP_LEN);

	for (int32_t i = 1; i < ctx->rounds; i++) {
		dec_keyexp += 4;
		for (int32_t j = 0; j < 4; j++)
			dec_keyexp[j] = AES_DEC_KEYEXP(dec_keyexp[j]);
	}
}
...
void conch_aes_encrypt(struct aes_ctx *ctx, uint8_t *state)
{
	...
	AES_ROUND_INIT(S, state, keyexp);

	int32_t i = ctx->rounds / 2;
	while (1) {
		keyexp += 4;
		AES_ENC_ROUND(T, S, keyexp);

		if (!--i)
			break;

		keyexp += 4;
		AES_ENC_ROUND(S, T, keyexp);
	}

	keyexp += 4;
	AES_ENC_END(S, T, keyexp);
	AES_OUT(state, S);
}

/* @func: conch_aes_decrypt
 * #desc:
...
{
	uint32_t *keyexp = (uint32_t *)ctx->dec_keyexp;
	uint32_t S[4], T[4];
	...
	while (1) {
		keyexp -= 4;
		AES_DEC_ROUND(T, S, keyexp);

		if (!--i)
			break;

		keyexp -= 4;
		AES_DEC_ROUND(S, T, keyexp);
	}

	keyexp -= 4;
	AES_DEC_END(S, T, keyexp);
	AES_OUT(state, S);
}
