...
{
	conch_memset(ctx, 0, sizeof(struct des_ctx));
...
void conch_des_crypto(struct des_ctx *ctx, uint8_t *buf, int32_t is_decrypt)
{
	...
	for (int32_t j = 1; j < 17; j++) {
		...
		conch_memset(ers, 0, sizeof(ers));
		_des_permute_sbox(ers, er);
		conch_memset(rn, 0, sizeof(rn));
