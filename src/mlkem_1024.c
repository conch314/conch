...
static void _poly_decompress_du(const uint8_t *a, struct poly *r)
{
	...
	for (int32_t i = 0; i < MLKEM_N; i += 8) {
		...
		for (int32_t j = 0; j < 8; j++) {
			r->coeffs[i + j] = ((uint32_t)(t[j] & 0x7ff)
				* MLKEM_Q + 1024) >> 11;
		}
	}
}
		...
		const struct polyvec *a, const struct polyvec *b)
{
	...
	for (int32_t i = 1; i < MLKEM_1024_K; i++) {
...
static void _sample_poly_eta1_prf(struct poly *r, const uint8_t *seed,
		uint8_t ran)
{
	uint8_t buf[MLKEM_1024_ETA1 * MLKEM_N / 4];
	SHA3_NEW(ctx);
...
/* @func: conch_mlkem1024_pke_genkey
 * #desc:
		...
		uint8_t *dkp)
{
	...
	for (int32_t i = 0; i < MLKEM_1024_K; i++) {
		for (int32_t j = 0; j < MLKEM_1024_K; j++)
	...
	conch_memcpy(ekp + MLKEM_1024_POLYVEC_BYTES, seed, MLKEM_SYM_LEN);
	/* dkp = s */
	_polyvec_tobytes(&s, dkp);
}

/* @func: conch_mlkem1024_pke_encrypt
 * #desc:
		...
		const uint8_t *msg, uint8_t *ct)
{
	...
	conch_memcpy(seed, ekp + MLKEM_1024_POLYVEC_BYTES, MLKEM_SYM_LEN);
	/* m = msg */
	_poly_frommsg(&m, msg);
	...
	_poly_compress_dv(&v, ct + MLKEM_1024_POLYVEC_COMPRESS);
}

/* @func: conch_mlkem1024_pke_decrypt
 * #desc:
		...
		uint8_t *ek, uint8_t *dk)
{
	...
	conch_mlkem1024_pke_genkey(ran, ek, dk);

	/* dk = dkp + ekp + hash_h(ekp) + ran2 */
	dk += MLKEM_1024_DKPKE_LEN;
	conch_memcpy(dk, ek, MLKEM_1024_EK_LEN);

	dk += MLKEM_1024_EK_LEN;
	_hash_h(ek, MLKEM_1024_EK_LEN, dk);

	dk += MLKEM_SYM_LEN;
	conch_memcpy(dk, ran2, MLKEM_RAN_LEN);
}

/* @func: conch_mlkem1024_encaps
 * #desc:
		...
		uint8_t *ct, uint8_t *sk)
{
	...
	conch_mlkem1024_pke_encrypt(r, ek, msg, ct);
}

/* @func: conch_mlkem1024_decaps
 * #desc:
...
void conch_mlkem1024_decaps(const uint8_t *dk, const uint8_t *ct,
		uint8_t *sk)
{
	uint8_t buf[MLKEM_SYM_LEN + MLKEM_1024_CT_LEN],
		ct2[MLKEM_1024_CT_LEN], sk2[MLKEM_SYM_LEN],
		msg[MLKEM_SYM_LEN];
	const uint8_t *k = buf;
	const uint8_t *r = buf + MLKEM_SYM_LEN;
	const uint8_t *dkp = dk;
	const uint8_t *ekp = dk += MLKEM_1024_DKPKE_LEN;
	const uint8_t *h = dk += MLKEM_1024_EK_LEN;
	const uint8_t *z = dk += MLKEM_SYM_LEN;

	/* sk2 = hash_j(z + ct) */
	conch_memcpy(buf, z, MLKEM_RAN_LEN);
	conch_memcpy(buf, ct, MLKEM_1024_CT_LEN);
	_hash_j(buf, MLKEM_RAN_LEN + MLKEM_1024_CT_LEN, sk2);

	/* msg = pke_decrypt(dkp, ct) */
	conch_mlkem1024_pke_decrypt(dkp, ct, msg);
	...
	conch_mlkem1024_pke_encrypt(r, ekp, msg, ct2);

	/* sk = if ct != ct2 : swap sk2 */
	uint32_t rr = 0;
	for (int32_t i = 0; i < MLKEM_1024_CT_LEN; i++)
		rr |= ct[i] ^ ct2[i];
