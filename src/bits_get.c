 ...
 */
uint32_t conch_bits_getfill(struct bits_get_ctx *ctx, const uint8_t *s,
		uint32_t len)
{
	uint32_t rem = ctx->len - ctx->pos;
	if (rem <= 4 && ctx->len > 4) {
		conch_memcpy(ctx->buf, ctx->buf + ctx->pos, rem);
		ctx->len = rem;
		ctx->pos = 0;
	}
...
#define MIN(a, b) (((a) < (b)) ? (a) : (b))
	...
	conch_memcpy(ctx->buf + ctx->len, s, rem);
	ctx->len += rem;
