/* @file: blowfish.c
 * #desc:
 *    The implementations of blowfish symmetric-key block cipher.
 *
 * #copy:
...
/* Original S-boxes */
static const uint32_t orig_S1[256] = {
...
static const uint32_t orig_S2[256] = {
...
static const uint32_t orig_S3[256] = {
...
static const uint32_t orig_S4[256] = {
...
#define F(s1, s2, s3, s4, x) \
	((s1[((x) >> 24) & 0xff] + s2[((x) >> 16) & 0xff]) \
	^ s3[((x) >> 8) & 0xff]) + s4[(x) & 0xff]


/* @func: conch_blowfish_encrypt
 * #desc:
...
{
	uint32_t Xl = *xl, Xr = *xr, t;

	for (int32_t i = 0; i < 16; i++) {
		Xl ^= ctx->P[i];
		Xr ^= F(ctx->S1, ctx->S2, ctx->S3, ctx->S4, Xl);
		t = Xl;
		Xl = Xr;
		Xr = t;
	}
		...
		uint32_t len)
{
	...
	for (int32_t i = 0; i < 18; i += 2) {
		conch_blowfish_encrypt(ctx, &xl, &xr);
		ctx->P[i] = xl;
		ctx->P[i + 1] = xr;
	}
...
void conch_blowfish_init(struct blowfish_ctx *ctx, const uint8_t *key,
		uint32_t len)
{
	conch_memcpy(ctx->S1, orig_S1, sizeof(orig_S1));
	conch_memcpy(ctx->S2, orig_S2, sizeof(orig_S2));
	conch_memcpy(ctx->S3, orig_S3, sizeof(orig_S3));
	conch_memcpy(ctx->S4, orig_S4, sizeof(orig_S4));

	/* len == 0 : skip set key */
	if (len)
		conch_blowfish_setkey(ctx, key, len);
}
