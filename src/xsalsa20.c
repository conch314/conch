/* @file: xsalsa20.c
 * #desc:
 *    The implementations of xsalsa20 stream encryption.
 *
 * #copy:
		...
		const uint8_t *ran)
{
	...
	state[7] = PACK4(ran + 4);
	state[8] = PACK4(ran + 8);
	state[9] = PACK4(ran + 12);

	for (int32_t i = 0; i < XSALSA20_ROUNDS; i += 2) {
		/* column */
		QR(state, 0, 4, 8, 12);
		QR(state, 5, 9, 13, 1);
		QR(state, 10, 14, 2, 6);
		...
		const uint8_t *ran, const uint8_t *ctr)
{
	...
	_hsalsa20_keygen(ctx->out.state, key, ran);
	...
	state[6] = PACK4(ran + 16);
	state[7] = PACK4(ran + 20);

	state[8] = PACK4(ctr);
	state[9] = PACK4(ctr + 4);
}

/* @func: conch_xsalsa20_block
 * #desc:
...
void conch_xsalsa20_crypto(struct xsalsa20_ctx *ctx, uint8_t *buf,
		size_t len)
{
	uint32_t n = XSALSA20_BLOCKSIZE;
	while (len) {
		if (len < n)
			n = len;

		conch_xsalsa20_block(ctx, XSALSA20_ROUNDS);
		for (uint32_t i = 0; i < n; i++)
			buf[i] ^= XSALSA20_KEYSTREAM(ctx, i);

		buf += n;
		len -= n;
		XSALSA20_COUNT1(ctx) += !++XSALSA20_COUNT0(ctx);
	}
}
