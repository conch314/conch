...
static void _scrypt_blockmix(uint8_t *b, uint8_t *y, uint32_t r)
{
	...
	conch_memcpy(t, y, SCRYPT_BLOCKSIZE * r);
}

/* @func: _scrypt_romix (static)
 * #desc:
 ...
 * #3: r [in]     block number
 * #4: v [out]    temporary buffer of v
 * #5: y [out]    temporary buffer of y
 */
		...
		uint32_t len)
{
	...
	conch_pbkdf2_sha256(pass, pass_len, b, b_len, dk, len, 1);
}
