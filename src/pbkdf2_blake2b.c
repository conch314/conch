		...
		uint32_t len, uint32_t k)
{
	...
	for (uint32_t i = 1; i <= n; i++) {
		...
		conch_hmac_blake2b_init(&ctx, pass, pass_len);
		conch_hmac_blake2b_process(&ctx, salt, salt_len);
		conch_hmac_blake2b_process(&ctx, count, 4);
		conch_hmac_blake2b_finish(&ctx);
		conch_memcpy(u, &(HMAC_BLAKE2B_STATE(&ctx, 0)),
			BLAKE2B_512_LEN);
		...
		for (uint32_t j = 1; j < k; j++) {
			conch_hmac_blake2b_init(&ctx, pass, pass_len);
			conch_hmac_blake2b_process(&ctx, u,
				BLAKE2B_512_LEN);
			conch_hmac_blake2b_finish(&ctx);
			conch_memcpy(u, &(HMAC_BLAKE2B_STATE(&ctx, 0)),
				BLAKE2B_512_LEN);
		...
		conch_memcpy(odk + f, tmp, ((f + BLAKE2B_512_LEN) > len) ?
