...
void conch_poly1305_init(struct poly1305_ctx *ctx, const uint8_t *key)
{
	...
	conch_memset(ctx->h, 0, sizeof(ctx->h));
	ctx->count = 0;
}

/* @func: conch_poly1305_block
 * #desc:
...
void conch_poly1305_tag(struct poly1305_ctx *ctx, uint8_t *tag)
{
	...
	conch_memcpy(tag, h, POLY1305_TAG_LEN);
}

/* @func: conch_poly1305_process
 * #desc:
...
#define BLOCKSIZE POLY1305_BLOCKSIZE
	...
	switch (n) {
		default:
			...
			/* processing */
			conch_poly1305_block(ctx, ctx->u.buf, 1);
			n = 0;
			s += h;
			len -= h;
		case 0:
...
void conch_poly1305_finish(struct poly1305_ctx *ctx)
{
	if (ctx->count) {
		...
		conch_poly1305_block(ctx, ctx->u.buf, 0);
	}

	conch_poly1305_tag(ctx, ctx->u.tag);
}
