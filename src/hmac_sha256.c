		...
		const uint8_t *key, uint32_t key_len)
{
	...
	conch_sha256_init(&sha256_ctx);

	if (key_len > SHA256_BLOCKSIZE) {
		conch_sha256(&sha256_ctx, key, key_len);
		key = &(SHA256_STATE(&sha256_ctx, 0));
		key_len = SHA256_LEN;
	}

	conch_memset(ctx->ipad, 0x36, SHA256_BLOCKSIZE);
	conch_memset(ctx->opad, 0x5c, SHA256_BLOCKSIZE);
	for (uint32_t i = 0; i < key_len; i++) {
		ctx->ipad[i] ^= key[i];
		ctx->opad[i] ^= key[i];
	}

	conch_sha256_init(&ctx->ipad_ctx);
	conch_sha256_init(&ctx->opad_ctx);
	conch_sha256_process(&ctx->ipad_ctx,
		ctx->ipad, SHA256_BLOCKSIZE);

	ctx->state = &(SHA256_STATE(&ctx->opad_ctx, 0));
}

/* @func: conch_hmac_sha256_process
 * #desc:
...
{
	conch_sha256_process(&ctx->ipad_ctx, s, len);
}

/* @func: conch_hmac_sha256_finish
 * #desc:
...
void conch_hmac_sha256_finish(struct hmac_sha256_ctx *ctx,
		uint64_t len)
{
	conch_sha256_finish(&ctx->ipad_ctx, len + SHA256_BLOCKSIZE);

	conch_sha256_process(&ctx->opad_ctx,
		ctx->opad, SHA256_BLOCKSIZE);
	conch_sha256_process(&ctx->opad_ctx,
		&(SHA256_STATE(&ctx->ipad_ctx, 0)), SHA256_LEN);
	conch_sha256_finish(&ctx->opad_ctx,
		SHA256_BLOCKSIZE + SHA256_LEN);
}

/* @func: conch_hmac_sha256
 * #desc:
...
void conch_hmac_sha256(struct hmac_sha256_ctx *ctx, const uint8_t *s,
		size_t len)
{
	conch_hmac_sha256_process(ctx, s, len);
	conch_hmac_sha256_finish(ctx, len);
}

/* @func: conch_hmac_sha224_init
 * #desc:
		...
		const uint8_t *key, uint32_t key_len)
{
	...
	conch_sha224_init(&sha224_ctx);

	if (key_len > SHA256_BLOCKSIZE) {
		conch_sha256(&sha224_ctx, key, key_len);
	...
	conch_sha224_init(&ctx->ipad_ctx);
	conch_sha224_init(&ctx->opad_ctx);
	conch_sha256_process(&ctx->ipad_ctx,
		ctx->ipad, SHA256_BLOCKSIZE);
...
/* @func: conch_hmac_sha224_process
 * #desc:
