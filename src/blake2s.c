...
int32_t conch_blake2s_init(struct blake2s_ctx *ctx, uint32_t dsize)
{
	...
	conch_memcpy(ctx->state, blake2s_iv, sizeof(ctx->state));
	ctx->tsize[0] = 0;
	ctx->tsize[1] = 0;
	ctx->flags[0] = 0;
	ctx->flags[1] = 0;
		...
		const struct blake2s_param *p)
{
	...
	/* parameters */
	conch_memcpy(&ctx->u, p, sizeof(ctx->u));
	for (int32_t i = 0; i < 8; i++)
		ctx->state[i] ^= ctx->u.state[i];
}

/* @func: conch_blake2s_process
 * #desc:
...
void conch_blake2s(struct blake2s_ctx *ctx, const uint8_t *s,
		size_t len)
{
	conch_blake2s_process(ctx, s, len);
