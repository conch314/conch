...
void test_aes_ecb(void)
{
	...
	len = 0;
	conch_aes_init(&ctx, g_buf, AES_128_TYPE);

	start = clock();
	for (int32_t i = 0; i < (1 << 21); i++) {
		conch_aes_encrypt(&ctx, g_buf);
		len += AES_BLOCKSIZE;
	}
	end = clock();
...
void test_aes_cbc(void)
{
	...
	start = clock();
	for (int32_t i = 0; i < (1 << 21); i++) {
		conch_aes_encrypt_cbc(&ctx, iv, g_buf);
		len += AES_BLOCKSIZE;
	}
	end = clock();
	time = (double)(end - start) / CLOCKS_PER_SEC;
	...
	start = clock();
	for (int32_t i = 0; i < (1 << 21); i++) {
		conch_aes_decrypt_cbc(&ctx, iv, g_buf);
		len += AES_BLOCKSIZE;
	}
	end = clock();
	time = (double)(end - start) / CLOCKS_PER_SEC;
...
void test_aes_cfb(void)
{
	...
	start = clock();
	for (int32_t i = 0; i < (1 << 21); i++) {
		conch_aes_encrypt_cfb(&ctx, iv, g_buf);
		len += AES_BLOCKSIZE;
	}
	end = clock();
	time = (double)(end - start) / CLOCKS_PER_SEC;
	...
	start = clock();
	for (int32_t i = 0; i < (1 << 21); i++) {
		conch_aes_decrypt_cfb(&ctx, iv, g_buf);
		len += AES_BLOCKSIZE;
	}
	end = clock();
	time = (double)(end - start) / CLOCKS_PER_SEC;
...
void test_aes_ofb(void)
{
	...
	for (int32_t i = 0; i < 32; i++) {
		conch_aes_crypto_ofb(&ctx, iv, g_buf, sizeof(g_buf));
		len += sizeof(g_buf);
	}
	end = clock();
	time = (double)(end - start) / CLOCKS_PER_SEC;
...
void test_aes_ctr(void)
{
	...
	start = clock();
	for (int32_t i = 0; i < 32; i++) {
		conch_aes_crypto_ctr(&ctx, iv, g_buf, sizeof(g_buf));
		len += sizeof(g_buf);
	}
	end = clock();
	time = (double)(end - start) / CLOCKS_PER_SEC;
...
void test_des(void)
{
	...
	len = 0;
	conch_des_init(&ctx, g_buf);

	start = clock();
	for (int32_t i = 0; i < (1 << 16); i++) {
		conch_des_crypto(&ctx, g_buf, 0);
		len += DES_BLOCKSIZE;
	}
	end = clock();
	time = (double)(end - start) / CLOCKS_PER_SEC;
...
void test_rc4(void)
{
	...
	len = 0;
	conch_rc4_init(&ctx, g_buf, 256);

	start = clock();
	for (int32_t i = 0; i < 128; i++) {
		conch_rc4_crypto(&ctx, g_buf, sizeof(g_buf));
		len += sizeof(g_buf);
	}
	end = clock();
	time = (double)(end - start) / CLOCKS_PER_SEC;
...
void test_chacha20(void)
{
	...
	len = 0;
	conch_chacha20_init(&ctx, g_buf, g_buf, g_buf);

	start = clock();
	for (int32_t i = 0; i < 128; i++) {
		conch_chacha20_crypto(&ctx, g_buf, sizeof(g_buf));
		len += sizeof(g_buf);
	}
	end = clock();
	time = (double)(end - start) / CLOCKS_PER_SEC;
...
void test_salsa20(void)
{
	...
	len = 0;
	conch_salsa20_init(&ctx, g_buf, g_buf, g_buf);

	start = clock();
	for (int32_t i = 0; i < 128; i++) {
		conch_salsa20_crypto(&ctx, g_buf, sizeof(g_buf));
		len += sizeof(g_buf);
	}
	end = clock();
	time = (double)(end - start) / CLOCKS_PER_SEC;
