...
{
	return conch_memcmp(a, b, len) ? "No" : "Yes";
}
...
void _test_p256(struct test_vector *t, int32_t n)
{
	...
	conch_ecdh_p256_public_key(t->in_a_pri, g_a_pub);
	conch_ecdh_p256_public_key(t->in_b_pri, g_b_pub);
	...
	conch_ecdh_p256_shared_key(t->in_a_pri, g_b_pub, g_buf);

	printf("out_a_key: %u -- %s\n ", t->out_key_len,
		verify_cmp(g_buf, t->out_key, t->out_key_len));
	print_hex(g_buf, t->out_key_len);

	conch_ecdh_p256_shared_key(t->in_b_pri, g_a_pub, g_buf);
