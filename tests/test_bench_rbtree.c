...
{
	int32_t key_a = RB_ENTRY(a, struct T, node)->key;
	int32_t key_b = RB_ENTRY(n, struct T, node)->key;
...
void test_rbtree(void)
{
	...
	start = clock();
	for (int32_t i = 0; i < SIZE; i++) {
		if (!conch_rb_insert(&root, &node[i].node, cmp))
	...
	/* first and last */
	p = conch_rb_first(&root);
	printf("first: k:%d %p\n", RB_ENTRY(p, struct T, node)->key, p);

	p = conch_rb_last(&root);
	printf("last: k:%d %p\n", RB_ENTRY(p, struct T, node)->key, p);

	/* search */
	start = clock();
	for (int32_t i = 0; i < SIZE; i++) {
		if (!conch_rb_search(&root, &node[i].node, cmp))
	...
	for (int32_t i = 0; i < A_SIZE; i++) {
		if (!conch_rb_erase2(&root, &node[i].node, cmp))
			printf("not found: i:%d k:%d\n", i, node[i].key);
	}
	end = clock();
	time = (double)(end - start) / CLOCKS_PER_SEC;
